...
// Google Solar measurements used by the AI estimate review.
...
exports.getSolarRoofData = onRequest({ cors: false, region: "us-central1", invoker: "public" }, async (req, res) => {
  permitCors(req, res);
  if (req.method === "OPTIONS") return res.status(204).send("");
  if (req.method !== "POST") return res.status(405).json({ error: "POST required." });
  try {
    ...
    if (!/^AIza[0-9A-Za-z_-]{30,}$/.test(apiKey)) return res.status(503).json({ error: "Google Solar API key is unavailable." });
    ...
    const insightsResponse = await fetch("https://solar.googleapis.com/v1/buildingInsights:findClosest?" + query.toString());
    ...
    try {
      ...
      const layersResponse = await fetch("https://solar.googleapis.com/v1/dataLayers:get?" + layerQuery.toString());
      dataLayersAvailable = layersResponse.ok;
    } catch (error) {
      logger.warn("Solar data-layer availability check failed", { message: error.message });
    }
    ...
    logger.error("Solar roof lookup failed", { statusCode, message: error.message });
    return res.status(statusCode).json({ error: statusCode === 401 ? "Sign in is required." : "Solar roof lookup failed." });
  }
});
