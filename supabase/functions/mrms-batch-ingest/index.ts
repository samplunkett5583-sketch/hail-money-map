...
const supabase = createClient(
  ...
  serverKey,
  {
    ...
    global: { fetch: supabaseServerFetch(serverKey) },
  },
);
...
function authorized(req: Request) {
  ...
  return batchToken.length >= 32 && supplied.length === batchToken.length && supplied === batchToken;
}
...
async function loadAnchors(date: string) {
  const [reports, evidence] = await Promise.all([
    supabase.from("hail_lsr_raw").select("lat,lon,hail_in,source").eq("event_date", date).gte("hail_in", 0.5).limit(5000),
    supabase.from("hail_ground_truth_evidence").select("lat,lon,hail_in,confidence").eq("event_date", date).eq("accepted", true).gte("confidence", 0.75).limit(1000),
  ]);
  ...
  return anchors.filter((row) => [row.lat, row.lon, row.hail_in, row.confidence].every(Number.isFinite));
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!validDate(date) || rows.length === 0 || rows.length > 25 || !Number.isInteger(totalRows) || totalRows < rows.length || totalRows > 5000) {
      return json({ error: "Invalid date or row count" }, 400);
    }
    for (let index = 0; index < rows.length; index += 1) {
      const row = rows[index];
      if (row?.event_date !== date || row?.source !== "mrms_mesh" || row?.source_product !== "MESH_Max_1440min" || !row?.polygon_geojson?.geometry) {
        return json({ error: `Invalid row ${index}` }, 400);
      }
    }
    const maxMesh = Number(payload?.max_mesh_in);
    if (!Number.isFinite(maxMesh) || maxMesh <= 0) return json({ error: "Invalid maximum hail size" }, 400);
    const { error: parentDayError } = await supabase.from("hail_radar_days").upsert(
      { event_date: date, max_mesh_in: maxMesh, source: "mrms_mesh", updated_at: new Date().toISOString() },
    ...
    const { error: upsertError } = await supabase.from("storm_swaths_canonical").upsert(
      canonicalRows,
    ...
    if (final) {
      const { error: cleanupError } = await supabase.from("storm_swaths_canonical").delete()
        .eq("event_date", date)
        .eq("source", "mrms_mesh")
