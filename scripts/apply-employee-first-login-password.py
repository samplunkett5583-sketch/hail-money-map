...
if old_login in public:
    public = public.replace(old_login, new_login, 1)
elif new_login not in public:
    raise SystemExit('Expected login block not found')

# Remove only the hard-coded seed map, preserving shared helpers.
functions = re.sub(
...
# Disable only the legacy test-login endpoint.
...
elif new_claims not in functions:
